...
grid = [[ 0 for i in range(9001)] for j in range(9001)]

def trace_wire_path(route, wire_value):
    north_south = 4000
    east_west = 4000
    for span in route.split(','):
        if span[0] == 'U':
            for _ in range(int(span[1:])):
                grid[north_south][east_west] += wire_value
                north_south += 1
        elif span[0] == 'D':
            for _ in range(int(span[1:])):
                grid[north_south][east_west] += wire_value
